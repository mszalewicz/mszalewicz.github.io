...
description: "Implementation of local IP traverse in Go."
pubDate: 2025-01-15
# heroImage: "../../assets/iterating_over_local_ip_addresses_in_go.webp"
...
```go {1-5}
func findAddresses() {
    interfaces, err := net.Interfaces() // [!code ++]

    if err != nil { // [!code highlight]
        log.Fatal(err) // [!code highlight]
    }

    for _, intrface := range interfaces { // [!code --]
        addresses, err := intrface.Addrs()
